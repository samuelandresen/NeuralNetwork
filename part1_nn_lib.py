...
class LinearLayer(Layer):
    ...
    def backward(self, grad_z):
        ...
        self._grad_W_current = np.transpose(self._cache_current[0]) @ grad_z
        self._grad_b_current = np.sum(grad_z, axis = 0)
        return grad_z @ np.transpose(self._cache_current[1])
    ...
    @property
    def grad_W_current(self):
        return self._grad_W_current
...
             - neurons {list} -- Number of neurons in each linear layer
        ...
        for i in range(len(neurons)):
            ...
            layer = LinearLayer(dim, neurons[i])
    ...
    def backward(self, grad_z):
        ...
        #                       ** START OF YOUR CODE **
        #######################################################################
        for (i, j) in reversed(self._layers):
            if j != None:
                grad_z = j.backward(grad_z)
            
            grad_z = i.backward(grad_z)
...
class Trainer(object):
    ...
    def shuffle(input_dataset, target_dataset):
        ...
        #                       ** START OF YOUR CODE **
        #######################################################################
        sorted = np.arange(len(input_dataset))
        np.random.shuffle(sorted)
        return input_dataset[sorted], target_dataset[sorted]
    ...
    def train(self, input_dataset, target_dataset):
        ...
        #                       ** START OF YOUR CODE **
        #######################################################################
        training_data_points, _ = np.shape(input_dataset)

        if training_data_points // self.batch_size > 1:
            batches = training_data_points // self.batch_size
        ...
        for i in range(self.nb_epoch):
            ...
            for j in range(batches):
                ...
                self.network.forward(input_dataset[start : end])
                self._loss_layer.forward(self.network.forward(input_dataset[start : end]), target_dataset[start : end])
